...
//   obs  -> actions         (used by OrtPolicyRunner)
//   time_step -> joint_pos, joint_vel, body_quat_w, ...
//
// This evaluator only uses the time_step->motion path.
...
public:
  ...
  void evaluate(float time_step, Eigen::VectorXf& joint_pos,
  ...
  void run(float time_step) {
    ...
    for (size_t i = 0; i < n_in; ++i) {
      tensors.push_back(Ort::Value::CreateTensor<float>(
          mem_, in_bufs_[i].data(), in_bufs_[i].size(), in_shapes_[i].data(),
          in_shapes_[i].size()));
    }

    auto outs = session_->Run(Ort::RunOptions{nullptr}, in_names_c_.data(),
                              tensors.data(), n_in, out_names_c_.data(), n_out);

    {
      auto shape = outs[jp_idx_].GetTensorTypeAndShapeInfo().GetShape();
      const int nj = static_cast<int>(shape.back());
